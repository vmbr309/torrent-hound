...
results_1337x = None
results, results_rarbg, exit, error_detected_rarbg, error_detected_tpb = (
    None,
    None,
    None,
    None,
...
num_results, num_results_rarbg, num_results_sky, num_results_1337x, print_version = (
    0,
    0,
    0,
    0,
...
ORDER_BY = enum(NAME=1, SIZE=3, UPLOADER=5, SEEDERS=7, LEECHERS=9, TYPE=13, UPLOADED=99)
...
def generateNewTorrentAPIToken(error=False, quiet_mode=False):
    ...
    refresh_url = (
        "https://torrentapi.org/pubapi_v2.php?get_token=get_token&app_id=" + str(app_id)
    )
    ...
    try:
        ...
        if "Connection aborted" in err_string:
            if quiet_mode == False:
                print(
                    colored.red(
                        "Server cannot be reached. Check Internet connectivity!"
                    )
                )
            # sys.exit(1)
...
def searchRarbg(search_string=defaultQuery, quiet_mode=False):
    ...
    options = (
        "format=json_extended&ranked=0&token="
        + str(auth_token)
        + "&app_id="
        + str(app_id)
    )
    ...
    try:
        ...
        if quiet_mode == False:
            ...
            if status_code == "429":  # Too Many Requests
                print(
                    colored.yellow(
                        "<HTTP 429> : Too Many Requests. Please try again after a while!"
                    )
                )
...
def parse_results_rarbg(response_json, quiet_mode=False, limit=10):
    ...
    if error_detected_rarbg == False:
        ...
        for post in response_json["torrent_results"][:limit]:
            ...
            s1 = temp_size.split(".")
            if len(s1[0]) == 4:
                res["size"] = humanize.naturalsize(
                    post["size"], binary=True, format="%.0f"
                )
            elif len(s1[1]) == 3:
                res["size"] = humanize.naturalsize(
                    post["size"], binary=True, format="%.1f"
                )
            else:
                res["size"] = temp_size
            ...
            try:
                res["ratio"] = format(
                    (float(res["seeders"]) / float(res["leechers"])), ".1f"
                )
...
def print_top_results_rarbg(limit=10):
    ...
    if results_rarbg != [] and results_rarbg != None:
        ...
        print(
            "{0} {1} {2} {3} {4} {5}".format(
                colored.red("No.").ljust(3),
                colored.red("Torrent Name").ljust(60),
                colored.red("File Size").rjust(10),
                colored.red("Seeders").rjust(7),
                colored.red("Leechers").rjust(6),
                colored.red("Ratio").rjust(6),
            )
        )
        index = 1
        for r in results_rarbg[:limit]:
            print(
                "{0} {1} {2} {3} {4} {5}".format(
                    ...
                    r["name"][:60].ljust(60),
                    r["size"].rjust(10),
                    str(r["seeders"]).rjust(6),
                    str(r["leechers"]).rjust(6),
                    str(r["ratio"]).rjust(8),
                )
            )
            index = index + 1
...
def pretty_print_top_results_rarbg(limit=10):
    global results_rarbg
    table_rarbg = VeryPrettyTable(
        left_padding_width=0, right_padding_width=0, padding_width=0
    )
...
def search1337x(
    search_string=defaultQuery, domain="1337x.to", quiet_mode=False, limit=10
):
    ...
    try:
        ...
        for row in rows[:limit]:
            ...
            row_data["seeders"] = int(
                row.find("td", {"class": "coll-2 seeds"}).text.strip()
            )
            row_data["leechers"] = int(
                row.find("td", {"class": "coll-3 leeches"}).text.strip()
            )
            try:
                row_data["ratio"] = format(
                    (float(row_data["seeders"]) / float(row_data["leechers"])), ".1f"
                )
            except ZeroDivisionError:
                row_data["ratio"] = "inf"
...
def searchSkyTorrents(
    search_string=defaultQuery,
    domain="skytorrents.lol",
    order_by=ORDER_BY_SKY.RELEVANCE,
    quiet_mode=False,
    limit=10,
):
    ...
    try:
        ...
        for trow in trows[:limit]:
            ...
            if (
                tds[0].findAll("img")[0].attrs["src"] == "/files/thumb_upm.png"
                and tds[0].findAll("img")[1].attrs["src"] == "/files/thumb_downm.png"
            ):
        ...
        if quiet_mode == False:
            if tbody == None:
                ...
                print(
                    colored.red("[SkyTorrents] Error : Unkown problem while searching")
                )
...
def pretty_print_top_results_skytorrents(limit=10):
    ...
    table_skytorrents = VeryPrettyTable(
        left_padding_width=0, right_padding_width=0, padding_width=0
    )
    no_str = str(colored.red("No"))
    name_str = str(colored.red("Torrent Name"))
...
def searchPirateBayCondensed(
    search_string=defaultQuery,
    page=0,
    order_by=ORDER_BY.SEEDERS,
...
def searchPirateBay(
    ...
    try:
        ...
        if "Operation timed out" in err_string:
            if domain == "thepiratebay.org":
                ...
                error_str = (
                    colored.yellow("[PirateBay] Error : Connection to ")
                    + colored.magenta(domain)
                    + colored.yellow(" timed out.\n")
                )
                error_str += (
                    colored.yellow("Trying to connect via ")
                    + colored.magenta(alternate_domain)
                    + colored.yellow("...")
                )
                if quiet_mode == False:
                    print(error_str)
                return searchPirateBay(
                    search_string=search_string, domain="piratebay.red"
                )
            ...
            if domain == "thepiratebay.org":
                tpb_working_domain = alternate_domain = "piratebay.red"
                error_str = (
                    colored.red("[PirateBay] Error : Connection to ")
                    + (domain)
                    + colored.red(" refused.\n")
                )
                error_str += (
                    colored.red("Trying to connect via ")
                    + (alternate_domain)
                    + colored.red("...")
                )
                if quiet_mode == False:
            ...
            if domain == "thepiratebay.org":
                ...
                error_str = (
                    colored.red("[PirateBay] Error : Connection to ")
                    + (domain)
                    + colored.red(" is probably blocked.\n")
                )
                error_str += (
                    colored.red("Trying to connect via ")
                    + (alternate_domain)
            ...
            error_str = (
                colored.red("[PirateBay] Unhandled Error : ")
                + colored.red(str(e))
                + colored.red("\nExiting...")
            )
            if quiet_mode == False:
    ...
    if table == None:
        if domain == "piratebay.red":
            error_string = (
                str(colored.yellow("[PirateBay] Error : No results found. "))
                + str(colored.magenta(domain))
                + str(colored.yellow(" might be unreachable!"))
            )
            if quiet_mode == False:
                print(error_string)
            ...
            error_string += (
                str(colored.yellow("\nTrying "))
                + str(colored.magenta(alternate_domain))
                + str(colored.yellow("..."))
            )
            if quiet_mode == False:
                print(error_string)
...
def _parse_search_result_table_row(tr):
    ...
    if link_name.contents == []:
        ...
        desc_string = (
            tds[1].find("font").contents[0].encode("utf-8").replace("&nbsp;", " ")
        )
        m = re.search(
            r"^Uploaded (Today|Y-day|\d\d-\d\d)\xc2\xa0(\d{4}|\d\d:\d\d), "
            + r"Size (\d+(?:.\d*)?\xc2\xa0(?:[KMG]iB))",
            desc_string,
        )
        try:
        ...
        if re.match(r"\d{4}", m.group(2)) == None:
            ...
            if m.group(1) == "Today":
                res["time"] = datetime.strptime(
                    now.strftime("%m-%d-%Y") + hour, "%m-%d-%Y %H:%M"
                )
            elif m.group(1) == "Y-day":
                res["time"] = datetime.strptime(
                    (now + timedelta(-1)).strftime("%m-%d-%Y") + hour, "%m-%d-%Y %H:%M"
                )
            else:
                res["time"] = datetime.strptime(
                    m.group(1) + "-" + str(now.year) + hour, "%m-%d-%Y %H:%M"
                )
        else:
        ...
        if res["magnet"] == res["link"]:  # Magnet link not found
            ...
            try:
                ...
                print(
                    colored.red(
                        "[PirateBay] Error : Unkown problem while searching for magnet link"
                    )
                )
                print(colored.yellow("ERR_MSG : " + str(e)))
        # print res
...
def searchPirateBayWithAPI(
    search_string=defaultQuery,
    sort_by=SORT_BY_TBP.SEEDS_DESC,
    domain="tpbc.herokuapp.com",
    quiet_mode=False,
):
    ...
    url = (
        base_url
        + "/search/"
        + removeAndReplaceSpaces(search_string)
        + "/?sort="
        + sort_by
    )
    tpb_url = url
...
def pretty_print_top_results_piratebay(limit=10):
    ...
    table_piratebay = VeryPrettyTable(
        left_padding_width=0, right_padding_width=0, padding_width=0
    )
    no_str = str(colored.red("No"))
    name_str = str(colored.red("Torrent Name"))
            ...
            if resNum <= 0 or resNum > num_results_1337x:
                ...
                if tpb_api == True:
                    if resNum <= num_results_rarbg:
                        ...
                        mLink = results_tpb_api[(resNum - 1) - num_results_rarbg][
                            "magnet"
                        ]
                    else:
                        # mLink = results_sky[(resNum-1)-num_results_tpb_api]['magnet']
                        mLink = results_1337x[(resNum - 1) - num_results_tpb_api][
                            "magnet"
                        ]
                else:
                    if resNum <= num_results_rarbg:
                ...
                if tpb_api == True:
                    if resNum <= num_results_rarbg:
                        ...
                        tLink = (
                            "https://"
                            + tpb_working_domain
                            + results_tpb_api[(resNum - 1) - num_results_rarbg]["link"]
                        )
                        ...
                        tLink = results_1337x[(resNum - 1) - num_results_tpb_api][
                            "link"
                        ]
                else:
                    if resNum <= num_results_rarbg:
...
def searchAllSites(query=defaultQuery, force_search=False, quiet_mode=False):
    ...
    if results_tpb_condensed == None or results_tpb_condensed == []:
        tpb_working_domain = "thepiratebay.zone"
        results_tpb_condensed = searchPirateBayCondensed(
            search_string=query, domain=tpb_working_domain, quiet_mode=quiet_mode
        )
...
def convertListJSONToPureJSON(result_list):
    ...
    if (
        result_list != [] and result_list != None
    ):  # Create a key 'results' only if there are some results
        result_json["results"] = {}
...
if __name__ == "__main__":
    ...
    # add arguments
    parser.add_argument(
        "query", help="Specify the search query", nargs="+", default=defaultQuery
    )
    parser.add_argument(
        "-q",
        "--quiet",
        help="Print output of search without any additional options",
        default=False,
        action="store_true",
    )
